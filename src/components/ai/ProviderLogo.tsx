...
const ProviderLogo: React.FC<ProviderLogoProps> = ({ 
  ...
  const getProviderIcon = () => {
    ...
    if (lowerProvider === 'openai') {
      ...
      const fillColor = theme === 'dark' ? 'fill-white' : 'fill-black';
      return (
        <div className={`${sizeClasses[size]} ${className} ${bgColor} rounded-sm flex items-center justify-center border border-border`}>
          <svg viewBox="0 0 24 24" className={`w-3/4 h-3/4 ${fillColor}`}>
    ...
    if (lowerProvider === 'google' || lowerProvider === 'gemini') {
      ...
      return (
        <div className={`${sizeClasses[size]} ${className} ${bgColor} rounded-sm flex items-center justify-center border border-border`}>
          <img src={geminiLogo} alt="Gemini" className="w-3/4 h-3/4 object-contain" />
        </div>
      );
    }
    ...
    if (lowerProvider === 'mistral') {
      ...
      return (
        <div className={`${sizeClasses[size]} ${className} ${bgColor} rounded-sm flex items-center justify-center border border-border`}>
          <img src={mistralLogo} alt="Mistral" className="w-3/4 h-3/4 object-contain" />
        </div>
      );
    }
